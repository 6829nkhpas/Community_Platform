  ...
  try {
    ...
    const updateData = {
      ...
      languages: Array.isArray(languages)
        ...
        : user.languages,
      experience: Array.isArray(experience) ? experience : user.experience,
      education: Array.isArray(education) ? education : user.education,
    };
...
router.post("/connect/:userId", authMiddleware, async (req, res) => {
  try {
    ...
    if (targetUser.blockedUsers.includes(currentUser._id)) {
      return res
        .status(400)
        .json({ message: "Cannot send connection request" });
    }
  ...
  try {
    ...
    if (isBlocked) {
      ...
      currentUser.connections = currentUser.connections.filter(
        (conn) => !conn.user.equals(targetUser._id)
      );
      targetUser.connections = targetUser.connections.filter(
        (conn) => !conn.user.equals(currentUser._id)
      );
...
router.get("/mutual/:userId", authMiddleware, async (req, res) => {
  try {
    const targetUser = await User.findById(req.params.userId).populate(
      "connections.user",
      "name avatar headline customUrl"
    );
...
router.put("/privacy", authMiddleware, async (req, res) => {
  try {
    const { profileVisibility, showActivity, showConnections } = req.body;
    ...
    user.privacy = {
      ...
      showActivity:
        showActivity !== undefined ? showActivity : user.privacy.showActivity,
      showConnections:
        showConnections !== undefined
          ? showConnections
          : user.privacy.showConnections,
    };
